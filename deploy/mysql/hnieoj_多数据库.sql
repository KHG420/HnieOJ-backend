...
CREATE TABLE `problem` (
  ...
  `spj_time_limit` int(11) DEFAULT '2000' COMMENT 'SPJ checker 时间限制 (ms)',
  `spj_memory_limit` int(11) DEFAULT '256' COMMENT 'SPJ checker 内存限制 (mb)',
  `spj_stack_limit` int(11) DEFAULT '128' COMMENT 'SPJ checker 栈限制 (mb)',
  `spj_output_limit` int(11) DEFAULT '16777216' COMMENT 'SPJ checker 输出限制 (bytes)',
  `spj_protocol` varchar(32) DEFAULT 'hnieoj-result-json-v1' COMMENT 'SPJ checker 结果协议',
  `interactor_code` longtext COMMENT '交互题 interactor 源代码内容',
  `interactor_language` varchar(20) DEFAULT NULL COMMENT '交互题 interactor 编译语言',
  `interactor_time_limit` int(11) DEFAULT '5000' COMMENT '交互题 interactor 时间限制 (ms)',
  `interactor_memory_limit` int(11) DEFAULT '256' COMMENT '交互题 interactor 内存限制 (mb)',
  `interactor_stack_limit` int(11) DEFAULT '128' COMMENT '交互题 interactor 栈限制 (mb)',
  `interactor_output_limit` int(11) DEFAULT '16777216' COMMENT '交互题 interactor 输出限制 (bytes)',
  `interactor_protocol` varchar(32) DEFAULT 'hnieoj-result-json-v1' COMMENT '交互题 interactor 结果协议',
...
CREATE TABLE `judge_node_token` (
  ...
  `last_seen_ip` varchar(64) DEFAULT NULL COMMENT '最近通过安全校验来源 IP',
  ...
  `supported_judge_modes` varchar(255) DEFAULT 'default' COMMENT '节点支持的判题模式，逗号分隔',
  `diagnostic_message` text DEFAULT NULL COMMENT '节点安全诊断信息',
  `cache_used_bytes` bigint(20) DEFAULT NULL COMMENT '测试数据缓存占用字节数',
  `cache_problem_count` int(11) DEFAULT NULL COMMENT '已缓存题目数量',
  `disk_total_bytes` bigint(20) DEFAULT NULL COMMENT '缓存挂载磁盘总字节数',
...
CREATE TABLE `judge` (
  ...
  `error_message` text,
  `diagnostic_message` text COMMENT '判题器/交互器诊断信息，仅教师及以上可见',
