...
- 本人资料修改保留三套契约，分工明确：`PUT /api/user/profile`（白名单字段直接修改，字段缺省保留、空串清除可选项）；通用申请 `POST/GET /api/user/profile/change-requests`（受理联系/社交字段：username/email/phone/avatar/qq/cf/github/blog，审核入口在 `/api/users` 下的 `GET /api/users/changes` 与 `PUT /api/users/{uid}/changes/approve`、`PUT /api/users/{uid}/changes/reject`、`PUT /api/users/changes/batch-approve`）；身份申请 `POST/GET /api/user/profile-change-requests`（独占受理实名/学院/年级/班级，提交后由 ADMIN/ROOT 审核，原值与新值以 JSON 保存，审核入口 `GET /api/admin/profile-change-requests` 与 `POST /api/admin/profile-change-requests/{id}/approve`、`POST /api/admin/profile-change-requests/{id}/reject`）。身份字段只能走身份申请，通用申请会拒绝携带身份字段的请求，避免两条流程互相覆盖。完整字段与错误码见 `docs/api-remaining.md`。
